...
def single_esn_testing(data_size=1000, washout=[0], hidden_size=60, input_size=1, output_size=3, load=None,
    ...
    if load is not None:
        ...
        model_esn = ESN(input_size, hidden_size, output_size, output_steps='mean', readout_training='cholesky').to(device)
    ...
    if gift_data is not None:
        ...
        X_data, _, Z_data = datasets.sin_link(data_size, c, d, e, c_change, d_change, e_change)
    X_data = torch.tensor(X_data).type(torch.DoubleTensor).to(device)
    Z_data = torch.tensor(Z_data).type(torch.DoubleTensor).to(device)
    X_data = X_data.reshape(-1, 1, save_input_size)
    Z_data = Z_data.reshape(-1, 1, save_output_size)
    ...
    if load is None:
        if feedback:
            file_loop = 'C_loop_' + str(hidden_size)
        else:
            file_loop = 'C_' + str(hidden_size)
