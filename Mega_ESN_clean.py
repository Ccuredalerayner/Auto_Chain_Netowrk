...
def pass_data(X_data, Y_data, Z_data, size_a, size_b, size_c):
    '''
    :return: X_data: [float] y values for dataset
    :return: Y_data: [[float]] a, b values for dataset
    :return: Z_data: [[float]] c, d, e values for data
    ...
    :return: Y_data_b: [[float]] a, b values for dataset manipulated for input of ESN B
    '''
    X_data = torch.tensor(X_data).type(torch.DoubleTensor).to('cuda')
    Y_data = torch.tensor(Y_data).type(torch.DoubleTensor).to('cuda')
    Z_data = torch.tensor(Z_data).type(torch.DoubleTensor).to('cuda')
    X_data = X_data.reshape(-1, 1, size_a)
    Y_data = Y_data.reshape(-1, 1, size_b)
    Z_data = Z_data.reshape(-1, 1, size_c)
    X_data = F.pad(input=X_data, pad=(0, 30, 0, 0), mode='constant', value=0)
    Y_data_b = F.pad(input=Y_data, pad=(0, 30, 0, 0), mode='constant', value=0)
    return X_data, Y_data, Z_data, Y_data_b
...
def train_new_network(X_data, Y_data, Z_data, Y_data_b,
                      ...
                      model_esn_a, model_esn_b, input_size_esn_a, input_size_esn_b):
    '''
    :return: X_data: [float] y values for dataset
    :return: Y_data: [[float]] a, b values for dataset
    :return: Z_data: [[float]] c, d, e values for data
    ...
    esn_loss_a, auto_loss_a = train_esn(X_data, Y_data, model_esn_a, model_auto_a, optimizer_a, input_size_esn_a)

    print(f'Mega training esn and auto: B')
    esn_loss_b, auto_loss_b = train_esn(Y_data_b, Z_data, model_esn_b, model_auto_b, optimizer_b, input_size_esn_b)
...
# loading
def load_files(load_date, input, inter, output):  # 31-08-2021_16-42-49
    '''
    ...
    model_esn_a, model_auto_a = load_network(load_file, type_a=True, input_size_a=input, output_size_a=inter,
                                             input_size_b=inter, output_size_b=output)

    # B file
    load_file = 'B_' + load_date
    model_esn_b, model_auto_b = load_network(load_file, type_a=False, input_size_a=input, output_size_a=inter,
                                             input_size_b=inter, output_size_b=output)
...
def Mega_esn(feedback=False, load=None, train=True, gift_data=None,
    ...
    X_data, Y_data, Z_data, Y_data_b = pass_data(gift_data[0], gift_data[1], gift_data[2], save_input_size_a,
                                                     save_input_size_b, save_output_size_b)
    ...
    if load is not None:
        model_esn_a, model_auto_a, model_esn_b, model_auto_b = load_files(load, save_input_size_a, save_input_size_b, save_output_size_b)
    ...
    for data, answer_a, answer_b in zip(data_a_x, guess_flat_a, guess_flat_b):
        ...
        hidden_layer_a = F.pad(input=hidden_a, pad=(save_input_size_a, 0, 0, 0), mode='constant', value=0)
        ...
        auto_ab_recon = F.pad(input=auto_ab_recon, pad=(save_output_size_a, 0, 0, 0), mode='constant', value=0)
        ...
        if feedback:
            ...
            hidden_layer_a = F.pad(input=auto_ba_recon, pad=(save_input_size_a, 0, 0, 0), mode='constant', value=0)
